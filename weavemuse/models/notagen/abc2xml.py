...
def abc_to_xml_file(abc_filename, output_dir='.', skip=0, num=1, rOpt=False, bOpt=False, fOpt=False):
    """
    Convert ABC file to XML file(s) directly in Python.
    
    Args:
        abc_filename (str): Path to the ABC file
        output_dir (str): Directory to write XML files (default: current directory)
        skip (int): Number of tunes to skip (default: 0)
        num (int): Maximum number of tunes to process (default: 1)
        rOpt (bool): Show whole measure rests in merged staffs
        bOpt (bool): Line break at EOL
        fOpt (bool): Force string/fret allocations for tab staves
    
    Returns:
        list: List of output XML filenames
    """
    # Read the ABC file
    abctext = readfile(abc_filename)
    
    # Get XML documents
    xml_docs = getXmlDocs(abctext, skip, num, rOpt, bOpt, fOpt)
    
    # Extract base filename
    fnm, ext = os.path.splitext(abc_filename)
    base_fnm = os.path.basename(fnm)
    ...
    for itune, xmldoc in enumerate(xml_docs):
        ...
        output_filename = base_fnm + fnmNum + '.xml'
        ...
        info('%s written' % output_path, warn=0)
